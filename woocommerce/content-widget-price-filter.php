...
<?php
$all_value = 'Все';
if (($current_min_price == $min_price) != 1 || ($current_max_price == $max_price) != 1) {
    $all_value = '$' . $current_min_price . ' - $' . $current_max_price;
}
?>
<p id="priceFilterToggle">Цена: <a class="btn-empty"><?= $all_value; ?> </a></p>
<form id="priceFilterModal" method="get" action="<?php echo esc_url($form_action); ?>">
    <div class="price_slider_wrapper">
        <div class="filter-price_inputs">
            <input type="number" id="iFilterMin" value="<?php echo esc_attr($current_min_price); ?>">
            <p>-</p>
            <input type="number" id="iFilterMax" value="<?= $current_max_price ?>">
        </div>
        <div class="price_slider" style="display:none;"></div>
        <div class="price_slider_amount" data-step="<?php echo esc_attr($step); ?>">
            <input style="display:block;" type="text" id="min_price" name="min_price"
                   value="<?php echo esc_attr($current_min_price); ?>" data-min="<?php echo esc_attr($min_price); ?>"
            ...
            <div class="price_label">
                <?php echo esc_html__('', 'woocommerce'); ?> <span class="from"></span> <span class="to"></span>
            </div>
