...
use Carbon_Fields\Container\Condition\Blog_ID_Condition;
use Carbon_Fields\Container\Condition\Boolean_Condition;
use Carbon_Fields\Container\Condition\Comparer\Any_Contain_Comparer;
use Carbon_Fields\Container\Condition\Comparer\Any_Equality_Comparer;
...
use Carbon_Fields\Container\Condition\Current_User_Capability_Condition;
use Carbon_Fields\Container\Condition\Current_User_ID_Condition;
use Carbon_Fields\Container\Condition\Current_User_Role_Condition;
use Carbon_Fields\Container\Condition\Post_Ancestor_ID_Condition;
use Carbon_Fields\Container\Condition\Post_Format_Condition;
use Carbon_Fields\Container\Condition\Post_ID_Condition;
use Carbon_Fields\Container\Condition\Post_Level_Condition;
use Carbon_Fields\Container\Condition\Post_Parent_ID_Condition;
use Carbon_Fields\Container\Condition\Post_Template_Condition;
use Carbon_Fields\Container\Condition\Post_Term_Condition;
use Carbon_Fields\Container\Condition\Post_Type_Condition;
use Carbon_Fields\Container\Condition\Term_Ancestor_Condition;
use Carbon_Fields\Container\Condition\Term_Condition;
use Carbon_Fields\Container\Condition\Term_Level_Condition;
use Carbon_Fields\Container\Condition\Term_Parent_Condition;
use Carbon_Fields\Container\Condition\Term_Taxonomy_Condition;
use Carbon_Fields\Container\Condition\User_Capability_Condition;
use Carbon_Fields\Container\Condition\User_ID_Condition;
use Carbon_Fields\Container\Condition\User_Role_Condition;
use Carbon_Fields\Container\Fulfillable\Translator\Array_Translator;
use Carbon_Fields\Container\Fulfillable\Translator\Json_Translator;
...
class Container_Condition_Provider implements ServiceProviderInterface {
	...
	protected function install_conditions( $ioc ) {
		...
		$cc_ioc['boolean'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Boolean_Condition();
		...
		$cc_ioc['post_id'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_ID_Condition();
		...
		$cc_ioc['post_parent_id'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Parent_ID_Condition();
			$condition->set_comparers( $ioc['container_condition_comparer_collections']['generic'] );
			return $condition;
		} );
		$cc_ioc['post_ancestor_id'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Ancestor_ID_Condition();
		...
		$cc_ioc['post_type'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Type_Condition();
		...
		$cc_ioc['post_format'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Format_Condition();
			$condition->set_comparers( $ioc['container_condition_comparer_collections']['nonscalar'] );
			return $condition;
		} );
		$cc_ioc['post_level'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Level_Condition();
			$condition->set_comparers( $ioc['container_condition_comparer_collections']['generic'] );
			return $condition;
		} );
		$cc_ioc['post_template'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Template_Condition();
			$condition->set_comparers( $ioc['container_condition_comparer_collections']['nonscalar'] );
			return $condition;
		} );
		$cc_ioc['post_term'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Post_Term_Condition( $ioc['wp_toolset'] );
			$condition->set_comparers( array(
		...
		$cc_ioc['term'] = $cc_ioc->factory( function() use ( $ioc ) {
			$condition = new Term_Condition( $ioc['wp_toolset'] );
			$condition->set_comparers( array(
				// Only support the custom comparer as this condition has its own comparison methods
				$ioc['container_condition_comparers']['custom'],
			) );
	...
	protected function install_comparers( $ioc ) {
		...
		$ioc['container_condition_comparers']['any_equality'] = function() {
			return new Any_Equality_Comparer();
		};
		...
		$ioc['container_condition_comparers']['any_contain'] = function() {
			return new Any_Contain_Comparer();
		};
	...
	protected static function install_translators( $ioc ) {
		...
		$ioc['container_condition_translator_json'] = function( $ioc ) {
			return new Json_Translator( $ioc['container_condition_factory'] );
		};
	}
